...
const WalletsAdd = () => {
  ...
  const createWallet = async () => {
    ...
    } else if (selectedWalletType === ButtonSelected.BITCOINIL) {
      if (selectedIndex === 2) {
        // zero index radio - HD segwit
        w = new HDSegwitP2SHWallet(true);
        w.setLabel(label || loc.wallets.details_title);
      } else if (selectedIndex === 1) {
        // btc was selected
        // index 1 radio - segwit single address
        w = new SegwitP2SHWallet(true);
        w.setLabel(label || loc.wallets.details_title);
      } else {
        // btc was selected
        // index 2 radio - hd bip84
        w = new HDSegwitBech32Wallet(true);
        w.setLabel(label || loc.wallets.details_title);
      }
      if (selectedWalletType === ButtonSelected.BITCOINIL) {
        if (entropy) {
          try {
            await w.generateFromEntropy(entropy);
          } catch (e) {
  ...
  const handleOnBitcoinilButtonPressed = () => {
    Keyboard.dismiss();
    setSelectedWalletType(ButtonSelected.BITCOINIL);
  };
    ...
    <ScrollView style={stylesHook.root}>
      ...
      <KeyboardAvoidingView enabled behavior={Platform.OS === 'ios' ? 'padding' : null} keyboardVerticalOffset={62}>
        ...
        <View style={styles.buttons}>
          ...
          <BitcoinilButton
            testID="ActivateBitcoinButton"
            active={selectedWalletType === ButtonSelected.BITCOINIL}
            onPress={handleOnBitcoinilButtonPressed}
            style={styles.button}
          />
          {/* <LightningButton
        ...
        <View style={styles.advanced}>
          {(() => {
            if ((selectedWalletType === ButtonSelected.ONCHAIN || selectedWalletType === ButtonSelected.BITCOINIL) && isAdvancedOptionsEnabled) {
              return (
                <View>
                  <BlueSpacing20 />
