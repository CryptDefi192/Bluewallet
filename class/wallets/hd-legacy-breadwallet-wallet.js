...
export class HDLegacyBreadwalletWallet extends HDLegacyP2PKHWallet {
  ...
  _calcNodeAddressByIndex(node, index, p2wpkh = false) {
    let _node;
    if (node === 0) {
      _node = this._node0 || (this._node0 = getBitcoinLib(this._isIl).bip32.fromBase58(this.getXpub()).derive(node));
    }
    if (node === 1) {
      _node = this._node1 || (this._node1 = getBitcoinLib(this._isIl).bip32.fromBase58(this.getXpub()).derive(node));
    }
    const pubkey = _node.derive(index).publicKey;
    const address = p2wpkh ? getBitcoinLib(this._isIl).payments.p2wpkh({ pubkey }).address : getBitcoinLib(this._isIl).payments.p2pkh({ pubkey }).address;
    return address;
  }
