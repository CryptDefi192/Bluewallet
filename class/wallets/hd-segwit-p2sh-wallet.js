...
export class HDSegwitP2SHWallet extends AbstractHDElectrumWallet {
  ...
  _getWIFByIndex(internal, index) {
    ...
    const root = getBitcoinLib(this._isIl).bip32.fromSeed(seed);
    const path = `m/49'/0'/0'/${internal ? 1 : 0}/${index}`;
    const child = root.derivePath(path);

    return getBitcoinLib(this._isIl).ECPair.fromPrivateKey(child.privateKey).toWIF();
  }
   ...
   * @returns {String}
   */
  static _nodeToP2shSegwitAddress(hdNode, isIl = false) {
    const { address } = getBitcoinLib(isIl).payments.p2sh({
      redeem: getBitcoinLib(isIl).payments.p2wpkh({ pubkey: hdNode.publicKey }),
    });
    return address;
  }
}
