...
import getBitcoinLib from '../../helpers/getbitcoinlib';
...
export class AbstractHDElectrumWallet extends AbstractHDWallet {
  ...
  _getNodeAddressByIndex(node, index) {
    ...
    let address;
    if (node === 0) {
      address = this.constructor._nodeToBech32SegwitAddress(this._node0.derive(index), this._isIl);
    }

    if (node === 1) {
      address = this.constructor._nodeToBech32SegwitAddress(this._node1.derive(index), this._isIl);
    }
  ...
  createTransaction(utxos, targets, feeRate, changeAddress, sequence, skipSigning = false, masterFingerprint) {
    ...
    let psbt = new getBitcoinLib(this._isIl).Psbt();
    let c = 0;
    const keypairs = {};
    const values = {};
    ...
    inputs.forEach(input => {
      ...
      if (!skipSigning) {
        ...
        keyPair = getBitcoinLib(this._isIl).ECPair.fromWIF(this._getWifForAddress(input.address));
        keypairs[c] = keyPair;
      }
  ...
  _addPsbtInput(psbt, input, sequence, masterFingerprintBuffer) {
    ...
    const p2wpkh = getBitcoinLib(this._isIl).payments.p2wpkh({ pubkey });
  ...
  combinePsbt(base64one, base64two) {
    const final1 = typeof base64one === 'string' ? getBitcoinLib(this._isIl).Psbt.fromBase64(base64one) : base64one;
    const final2 = typeof base64two === 'string' ? getBitcoinLib(this._isIl).Psbt.fromBase64(base64two) : base64two;
   ...
   * @returns {String}
   */
  static _nodeToBech32SegwitAddress(hdNode, isIl = false) {
    return getBitcoinLib(isIl).payments.p2wpkh({
      pubkey: hdNode.publicKey,
    }).address;
  }

  static _nodeToLegacyAddress(hdNode, isIl = false) {
    return getBitcoinLib(isIl).payments.p2pkh({
      pubkey: hdNode.publicKey,
    }).address;
  }
    ...
    for (let cc = 0; cc < psbt.inputCount; cc++) {
      ...
      if (!psbt.inputHasHDKey(cc, hdRoot)) {
        for (const derivation of psbt.data.inputs[cc].bip32Derivation || []) {
          ...
          const keyPair = getBitcoinLib(this._isIl).ECPair.fromWIF(wif);
          try {
   ...
   */
  static seedToFingerprint(seed, isIl = false) {
    const root = getBitcoinLib(isIl).bip32.fromSeed(seed);
   ...
   */
  static mnemonicToFingerprint(mnemonic, isIl = false) {
    const seed = bip39.mnemonicToSeedSync(mnemonic);
    return AbstractHDElectrumWallet.seedToFingerprint(seed, isIl);
  }
  ...
  getMasterFingerprintHex() {
    const seed = this._getSeed();
    return AbstractHDElectrumWallet.seedToFingerprint(seed, this._isIl);
  }
}
