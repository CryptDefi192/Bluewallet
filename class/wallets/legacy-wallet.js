...
export class LegacyWallet extends AbstractWallet {
  ...
  async generate() {
    ...
    this.secret = getBitcoinLib(this._isIl).ECPair.makeRandom({ rng: () => buf }).toWIF();
  }
  ...
  async generateFromEntropy(user) {
    ...
    do {
      ...
      try {
        this.secret = getBitcoinLib(this._isIl).ECPair.fromPrivateKey(buf).toWIF();
        return;
      } catch (e) {
        if (i === 5) throw e;
      }
  ...
  getAddress() {
    ...
    try {
      ...
      address = getBitcoinLib(this._isIl).payments.p2pkh({
  ...
  isAddressValid(address) {
    try {
      getBitcoinLib(this._isIl).address.toOutputScript(address);
      return true;
    } catch (e) {
      return false;
    }
