...
export class HDLegacyP2PKHWallet extends AbstractHDElectrumWallet {
  ...
  _getNodeAddressByIndex(node, index) {
    ...
    let address;
    if (node === 0) {
      address = this.constructor._nodeToLegacyAddress(this._node0.derive(index), this._isIl);
    }

    if (node === 1) {
      address = this.constructor._nodeToLegacyAddress(this._node1.derive(index), this._isIl);
    }
